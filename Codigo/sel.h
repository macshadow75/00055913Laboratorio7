...
//Matriz BVAReta
void calculateBVARetaMatrix(Matrix &B){
    zeroes(B,2,6);
...
Matrix createLocalK(int e,mesh &m){
    float T1,K1,L1,D1,E1,Ae,J,D;
    Matrix matrixM,matrixN,matrixO,matrixP, matrixQ, matrixAe;
    Matrix K,g_matrix,g_matrix_t,Alpha,BVAReta,Alphat,BVARetat,BPrima,BPrimat;
    ...
    calculateBVARetaMatrix(BVAReta);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(BVAReta,BVARetat);
    calculateBPrima(BPrima);
    transpose(BPrima,BPrimat);
    transpose(g_matrix,g_matrix_t);

    T1 = m.getParameter(CONSTANT_T);
    productRealMatrix(T1*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BVAReta,2,2,6),6,2,6),matrixM);
    K1 = m.getParameter(CONSTANT_K);
    productRealMatrix(K1*Ae/(D*D),productMatrixMatrix(BVARetat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BVAReta,2,2,6),2,2,6),6,2,6),matrixN);
    D1 = m.getParameter(CONSTANT_D);
    productRealMatrix((-1)*(D1*Ae)/(8*D*D),productMatrixMatrix(BVARetat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixAe);
    L1 = m.getParameter(CONSTANT_L);
    productRealMatrix((J*L1)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixQ);

    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixP);

    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixM,matrixN,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixQ,matrixAe,6,3));
    ubicarSubMatriz(K,6,8,0,5,matrixP);
    return K;
}
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(CONSTANT_PX);
    float f_y = m.getParameter(CONSTANT_PY);
    float E1 = m.getParameter(CONSTANT_E);
